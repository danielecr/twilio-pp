...
// TestUpsertBatch_PopulatesTwilioCareersJobsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed twilio_careers_jobs table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesTwilioCareersJobsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("twilio-careers-jobs", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "twilio-careers-jobs").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "twilio_careers_jobs")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count twilio_careers_jobs: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("twilio_careers_jobs count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

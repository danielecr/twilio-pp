...
// TestMigrate_AddsColumnsOnUpgrade_TwilioCareersJobs verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_TwilioCareersJobs(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "twilio_careers_jobs" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("twilio_careers_jobs")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from twilio_careers_jobs after migrate", want)
		}
	}
}

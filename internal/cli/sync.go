...
import (
	...
	"sync/atomic"
	"time"
	"twilio-jobs-pp-cli-pp-cli/internal/cliutil"
...
Resource scoping:
  ...
  twilio-jobs-pp-cli-pp-cli sync

  # Sync specific resources only
  twilio-jobs-pp-cli-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  twilio-jobs-pp-cli-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  twilio-jobs-pp-cli-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  twilio-jobs-pp-cli-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  twilio-jobs-pp-cli-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/twilio-jobs-pp-cli-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "twilio-careers-jobs":
		return db.UpsertTwilioCareersJobs(data)
	default:

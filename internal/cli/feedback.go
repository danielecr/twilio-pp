...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TWILIO_JOBS_PP_CLI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TWILIO_JOBS_PP_CLI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "twilio-jobs-pp-cli-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.twilio-jobs-pp-cli-pp-cli/feedback.jsonl.
When ` + "`TWILIO_JOBS_PP_CLI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TWILIO_JOBS_PP_CLI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  twilio-jobs-pp-cli-pp-cli feedback list
  twilio-jobs-pp-cli-pp-cli feedback list --limit 5
  twilio-jobs-pp-cli-pp-cli feedback list --json`,
